...
export class CreatePostHandler implements ICommandHandler<CreatePostCommand>, OnModuleInit {
  ...
  async execute(command: CreatePostCommand): Promise<string> {
    ...
    this.logger.log(`[Post-MS][Handler] CreatePostHandler.execute entered for ownerId=${ownerId}`);

    // 1. Асинхронная валидация файлов через gRPC
    for (const imgDto of images) {
      try {
        const response = (await firstValueFrom(
          this.fileService.getFileStatus({ fileId: imgDto.fileId }),
        )) as any;

        if (!response || !response.file) {
          throw new DomainException({
            code: DomainExceptionCode.BadRequest,
            message: `File ${imgDto.fileId} not found`,
          });
        }
        ...
        if (response.file.status !== FileStatus.UPLOADED) {
          throw new DomainException({
            code: DomainExceptionCode.BadRequest,
            message: `File ${imgDto.fileId} is not uploaded yet`,
          });
        }
        ...
        if (response.file.ownerId !== ownerId) {
          throw new DomainException({
            ...
            message: `File ${imgDto.fileId} does not belong to user`,
          });
        }
      } catch (error) {
        ...
        throw new DomainException({
          code: DomainExceptionCode.BadRequest,
          message: `Error validating file ${imgDto.fileId}: ${error.message}`,
        });
      }
    }
    ...
    // 3. Сохранение в БД
    this.logger.log('[Post-MS][Handler] before postRepository.createPost');
    await this.postRepository.createPost(postEntity);
    this.logger.log('[Post-MS][Handler] after postRepository.createPost');
